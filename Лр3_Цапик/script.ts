...
function task1(a: number, b: number): number {
    //Вивести на екран більше з двох даних чисел
    return Math.max(a, b);
}

function task2(a: number): number {
    //Поміняти місцями першу і останню цифри числа
    const str = String(a);
    switch (str.length) {
        case 1:
...
function task3(str: string): number {
    //Скільки букв "у" в слові стоїть на парних місцях?
    str = str.toLowerCase();
    let acc = 0;
...
function task4(name: string, surname: string): string {
...
function task5(a: number[]): boolean {
    //Для цілочисельного масиву А, що містить 10 елементів, визначити, чи кратна сума його елементів 7
...
function task6(matrix: number[][]): number[] {
    //Дана матриця Y (m, n). Знайти стовпець з найбільшою і найменшою сумою елементів
    const sum = [];
...
function randArr(m: number, n: number = 1) {
    const arr = [];
    for (let i = 0; i < m; i++) {
        if (n !== 1) {
            arr[i] = [];
...
function myPrompt(message: string, conditionFunction = (result: string) => {
    return true;
}) {
...
function is3Length(str: string): boolean {
    return str.length >= 3;
}